...
==== Download and Install JBoss EAP 6.4

The unit tests require a local installation of JBoss EAP 6.4.  In the following steps, you will download and install JBoss EAP. Then run the unit tests.

To run unit-tests you will run the `jboss-eap` locally on your machine. The path for which can be configured through `arquillian.xml`

. Follow the directions at https://access.redhat.com/documentation/en-us/red_hat_jboss_enterprise_application_platform/6.4/html/getting_started_guide/sect-download_and_install_jboss_eap_using_the_graphical_installation_program[this link to download and install JBoss EAP 6.4.]

. Move back to JBDS and update the configuration file, with the local of your JBoss EAP installation

.. Edit file: monolith-coolstore-service/src/test/resources/arquillian.xml

.. Replace
+
[source,xml]
----
      <property name="jbossHome">/home/jasingh/EAP/jboss-eap-6.4</property>
----
...
[source,xml]
----
      <property name="jbossHome">${LOCATION-OF-YOUR-JBOSS-EAP-INSTALL-DIRECTORY}</property>
----

NOTE: Be sure to update this with the location of your JBoss EAP installation directory.
...
<datasource jndi-name="java:jboss/datasources/CoolstoreDS" pool-name="CoolstoreDS" enabled="true" use-java-context="true">
    <connection-url>jdbc:postgresql://172.17.0.2:5432/monolith</connection-url>
