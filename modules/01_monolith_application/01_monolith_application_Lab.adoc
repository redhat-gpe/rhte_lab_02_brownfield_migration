...
The unit tests require a local installation of https://access.redhat.com/documentation/en-us/red_hat_jboss_enterprise_application_platform/6.4/html/getting_started_guide/sect-download_and_install_jboss_eap_using_the_graphical_installation_program[JBoss EAP 6.4].  The client VM comes preloaded with local copy of JBoss EAP 6.4 server at location `/opt/eap`.
...
=== DataBase Deployment
* The client VM comes preinstalled with `postgresql` with properties as
** Username: jboss
** Password: jboss
** database: monolith
