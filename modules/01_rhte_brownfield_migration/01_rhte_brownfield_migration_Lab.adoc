...
+
NOTE: For the labs, the output from running the above rule is provided through http://rhamt-httpd-server.apps.f4b9.rhte.opentlc.com[URL^]. Due to the limitation to keep clientvm same across different rhte labs, the reports have been pre-generated and packaged in a http server.
The steps to package can be found in Appendix section <<_packing_rhamt_reports_in_http_container,Packing RHAMT Reports in HTTP Container>>
