...
** password: r3dh4t1!
** server-url: https://master.40b3.rhte.opentlc.com, where `40b3` is the unique ID for this shared environment
...
[source,sh]
----
ssh lab-user@clientvm$LAB_USER_NUM.40b3.rhte.opentlc.com
----
...
[source,sh]
----
$ oc login https://master.40b3.rhte.opentlc.com/ -uuser$LAB_USER_NUM -pr3dh4t1!
----
...
----
* substitue $USER for your LAB username.
...
+
NOTE: For the labs, the output from running the above rule is provided through http://rhamt-httpd-server.apps.40b3.rhte.opentlc.com[URL^]
...
=== Review the Reports
. Open the in the http://rhamt-httpd-server.apps.40b3.rhte.opentlc.com[URL^] browser
...
. View the reports in browser by opening the  http://rhamt-web-server.apps.40b3.rhte.opentlc.com[URL^]
...
$ oc projects
----
* Expect to see a project named `rhte-mw-bfield-migration-constraints-$GUID`, where "$GUID" is the unique identifier you received in the provisioning email for the labs.

. Select the `rhte-mw-bfield-migration-constraints-$USER_NUM` project:
+
[source,sh]
----
...
[source,sh]
----
$ oc get limits rhte-mw-bfield-migration-constraints-$GUID-core-resource-limits -o yaml
----
+
.Sample Output
[source,yaml]
...
$ git remote add gogs http://amm-gogs-rhte-migration.apps.40b3.rhte.opentlc.com/$GOGS_USER/rhte-brownfield-app-migration.git
----
