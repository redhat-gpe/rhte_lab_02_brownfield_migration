...
= Brownfield Migration to OpenShift Lab
...
The lab environment consists of a remote virtual machine with the following tooling pre-installed :

* *Client side tooling*:
+
ie; `mvn`,`rhamt`,`oc client`, `git` etc

* *OpenShift Container Platform*.

This section guides you through the procedure to order a course virtual machine.

There are two options for doing so. [red]#Execute only one of the options below:#
...
. Select the following catalog item: `Brownfield Migration to OpenShift`
+
...
. Notice that the GUID grabber returns a page with instructions on how to SSH into your remote virtual machine.
+
Do so exactly as instructed by the GUID grabber.
...
=== Lab Asset Source Code

. To help you get started, lab assets are made available to you in Gtihub.  Please clone these lab assets as follows:
+
[source,sh]
----
$ mkdir -p $HOME/labs
...
----
. Set the following shell environment variables:
+
[source,sh]
----
$ export OCP_USERNAME=developer
$ export GUID=<course VM GUID>
----
...
The ocp cluster can be accessed by running the following command:

[source,sh]
----
$ oc login https://$HOSTNAME:8443 -u $OCP_USERNAME -pr3dh4t1!
----

The lab comes with a pre-created openshift project `rhte-mw-bfield-migration-constraints-$OCP_USERNAME`

WARNING: This is a special openshift project with constrained resources and will be used for <<_openshift_resources,Openshift Resources>>

==== OCP Web Console
. Using your browser, open the URL retrieved from executing the following:
+
-----
$ echo  -en "\n\nhttps://clientvm.$GUID.openshift.opentlc.com:8443\n\n"
-----


. Authenticate using the same credentials used when authenticating from the `oc` utility.
...
=== Build Application
. Execute a maven build of the lab assets:
+
[source,sh]
----
...
[source,sh]
----
$ oc login https://master.$GUID.rhte.opentlc.com -u developer -pr3dh4t1!
----
. Create a openshift project for the deployment of monolith-coolstore.
+
[source,sh]
----
$ oc new-project $OCP_USERNAME-monolith-coolstore
----
...
$ mvn clean package -DskipTests
----
. The above command will create java war file at `~/labs/monolith-coolstore/deployments`
+
----
$ ls -l deployments
...
+
NOTE: For the labs, the output from running the above rule is provided through URL: http://rhamt-reports-httpd-server.apps.$GUID.rhte.opentlc.com . Due to the limitation to keep clientvm same across different rhte labs, the reports have been pre-generated and packaged in a http server.
...
$ oc projects
----
* Expect to see a project named `rhte-mw-bfield-migration-constraints-$OCP_USERNAME` .

. Select the `rhte-mw-bfield-migration-constraints-$OCP_USERNAME` project:
+
[source,sh]
----
...
[source,sh]
----
$ oc get limits rhte-mw-bfield-migration-constraints-$OCP_USERNAME-core-resource-limits -o yaml
----
+
.Sample Output
[source,yaml]
...
[source,sh]
----
$ oc new-project $OCP_USERNAME-modern-inventory-service
----
...
[source,sh]
----
$ oc new-project $OCP_USERNAME-modern-catalog-service
----
...
[source,sh]
----
$ export CART_PRJ=$OCP_USERNAME-modern-cart-service
...
----
* Use the internal catalog service routing URL as shown here `http://catalog-service.$OCP_USERNAME-modern-catalog-service.svc.cluster.local:8080`
...
. Download the `RHAMT` cli tool from URL: https://developers.redhat.com/products/rhamt/download/
