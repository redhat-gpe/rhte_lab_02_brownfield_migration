...
. In the POM file of the inventory service project, add the `org.wildfly.swarm:keycloak` dependency.
...
You deployed the catalog service on OpenShift in previous labs. In this section, you deploy and test the secured version of the inventory service.
...
. Add the secure endpoint configuration to the `catalog-service` ConfigMap in the OpenShift inventory service project.
...
[source,text]
----
$ export CATALOG_URL=http://$(oc get route inventory-service -n $SECURE_CATALOG_PRJ -o template --template='{{.spec.host}}')
